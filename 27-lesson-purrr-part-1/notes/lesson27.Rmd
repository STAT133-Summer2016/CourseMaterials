...
```{r}
x1 <- data.frame(x1) %>% setNames(letters[1:3])
```

Use map_dbl() to find column means of `x1`
```{r}
# Your code here
```

Use map_dbl() to column medians of `x1`
```{r}
# Your code here
```

Use map_dbl() to find column standard deviations of `x1`
```{r}
# Your code here
```
